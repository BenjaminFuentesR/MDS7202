...
with gr.Blocks(title = "ChaucherApp - Soporte") as app:
    ...
    with gr.Row():
        ...
        with gr.Column(variant = "panel"):
            ...
            asunto_input = gr.Textbox(
                label = "Asunto",
                placeholder = "Ejemplo: Transferencia no recibida y...",
                lines = 1)
            
            contenido_input = gr.Textbox(
                label = "Contenido",
                placeholder = "Mensaje exacto enviado por el cliente...",
                lines = 5
            )
    ...
    btn_predecir.click(
        ...
        inputs = [asunto_input, contenido_input, canal_input, categoria_input],
        outputs = output
    )
